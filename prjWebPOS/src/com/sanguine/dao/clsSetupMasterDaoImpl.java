...
public class clsSetupMasterDaoImpl implements clsSetupMasterDao{
	...
	public clsPropertySetupModel funGetObjectPropertySetup(String propertyCode,
		...
		//String sql="Select  from tblpropertysetup where strPropertyCode='"+propertyCode+"' and  strClientCode = '"+clientCode+"' ";
	/*	//  This is for getting all cell data of the row in tblPropertysetup
		String hql="FROM clsPropertySetupModel  where strPropertyCode= :propertyCode AND strClientCode= :clientCode ";
		Query query = sessionFactory.getCurrentSession().createQuery(hql);
		query.setParameter("clientCode",clientCode);
		query.setParameter("propertyCode",propertyCode);
		List results = query.list();	
		if(results.size()>0){
			return (clsPropertySetupModel)results.get(0);
		}else{
			return null;
		}
		*/
