...
//	private SessionFactory CRMSessionFactory;
	...
	{
		long lastNo=0;
		if (req.getSession().getAttribute("selectedModuleName").toString()
				.equalsIgnoreCase("1-WebStocks")) {
	...
	{
	    sql += " where a.strNonStockableItem='Yes' ";
	}
	...
	try
	{
		...
		/*
		 * String hql=
		 * "from clsPropertyMaster a, clsUserMasterModel b where a.strPropertyCode=b.strProperty "
		 * + " and b.strUserCode='"+userCode+"' and  a.strClientCode='"+
		 * clientCode+"' and b.strClientCode='"+clientCode+"'";
		 * System.out.println(hql); Query
		 * query=sessionFactory.getCurrentSession().createQuery(hql);
		 * List mapProperty= query.list(); for(int
		 * i=0;i<mapProperty.size();i++) { Object[]
		 * ob=(Object[])mapProperty.get(i); clsPropertyMaster
		 * PropertyMaster=(clsPropertyMaster)ob[0];
		 * System.out.println("Prop Code="
		 * +PropertyMaster.getPropertyCode
		 * ()+"\tName="+PropertyMaster.getPropertyName());
		 * map.put(PropertyMaster.getPropertyCode(),
		 * PropertyMaster.getPropertyName()); }
		 */
		...
		String sql = " select a.strPropertyCode,a.strPropertyName,b.strModule " + " from tblpropertymaster a, tbluserlocdtl b "
				   + " where a.strPropertyCode=b.strPropertyCode " + " and b.strUserCode='" + userCode + "' "
				   + " and  a.strClientCode='" + clientCode + "' " + " and b.strClientCode='" + clientCode + "'"
				   + " group by b.strModule, a.strPropertyCode  ";
...
//    {
//	Query query = webBooksSessionFactory.getCurrentSession().createQuery("delete clsTempLedgerModel where strUserCode = :userCode and strClientCode= :clientCode and strPropertyCode =:propertyCode ");
//	query.setParameter("clientCode", clientCode);
//	query.setParameter("userCode", userCode);
//	query.setParameter("userCode", userCode);
//	return query.executeUpdate();
...
//    {
//    	String sql = "insert into tblcurrentledger (strDebtorCode,strDebtorName,strClientCode,strUserCode,strPropertyCode) " + " select a.strDebtorCode,a.strDebtorName,'" + clientCode + "','" + userCode + "','"+propertyCode+"' from tblsundarydebtormaster a ";
