...
public class clsPOSWebPOSLoginController {
	...
	public ModelAndView funLoginCheck(@RequestParam("userName") String userCode,@RequestParam("password") String password
	...
	{     
		...
		try
		{
			...
			String posUrl = 	clsPOSGlobalFunctionsController.POSWSURL+"/APOSIntegration/funAuthenticateUser?strUserCode="
				+ ""+userCode+"&strPassword="+password;
	        ...
	        JSONObject jObjLoginStatus=(JSONObject)jArrLoginStatus.get(0);*/
	        String status=jObjLoginStatus.get("Status").toString();
	        String userName=jObjLoginStatus.get("UserName").toString();
	        String userType=jObjLoginStatus.get("SuperType").toString();
	...
	private Map<Integer,clsPOSMenu> funGetPOSMenuMap(String userCode,String clientCode,String moduleType,String superUser,String POSCode) throws Exception 
	{
		...
		String posUrl = 	clsPOSGlobalFunctionsController.POSWSURL+"/APOSIntegration/funGetMainMenu"
			+ "?UserCode="+userCode+"&ModuleType="+moduleType+"&clientCode="+clientCode+"&SuperUser="+superUserYN+"&POSCode="+POSCode;
		System.out.println(posUrl);
